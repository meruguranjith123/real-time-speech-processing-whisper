...
# Audio accumulation buffer for each client session
audio_buffers = {}  # {session_id: {'audio': [], 'sample_rate': 16000, 'last_process': time}}
...
def handle_audio(audio_data):
    """
    Handle incoming audio data from client - accumulate chunks before processing
    
    Args:
        audio_data: Base64 encoded audio data (PCM format from browser)
    """
    import time
    session_id = request.sid
    ...
    try:
        ...
        # FORCE PCM - if format is missing or webm, try to detect PCM
        if audio_format == 'webm' or audio_format == 'unknown':
            # Try to detect if it's actually PCM by checking if it can be parsed as int16
            try:
                test_array = np.frombuffer(audio_bytes[:100], dtype=np.int16)
                if len(test_array) > 0:
                    audio_format = 'pcm'
            except:
                pass
        ...
        if audio_format == 'pcm':
            ...
            try:
                ...
                buffer['audio'].append(audio_chunk)
                
                # Process accumulated audio every 2 seconds or when buffer is large enough
                current_time = time.time()
                time_since_last = current_time - buffer['last_process']
                total_samples = sum(len(chunk) for chunk in buffer['audio'])
                min_samples_for_processing = sample_rate * 2  # 2 seconds of audio
                
                # Process if we have enough audio (2+ seconds) or it's been 3 seconds
                if total_samples >= min_samples_for_processing or time_since_last >= 3.0:
                    # Concatenate all accumulated chunks
                    if buffer['audio']:
                        audio_array = np.concatenate(buffer['audio'])
                        buffer['audio'] = []  # Clear buffer
                        buffer['last_process'] = current_time
                        
                        logger.info(f"Processing accumulated audio: {len(audio_array)} samples ({len(audio_array)/sample_rate:.2f}s) at {sample_rate}Hz")
                    ...
                    # Just acknowledge receipt, don't process yet
                    logger.debug(f"Accumulating audio: {total_samples} samples ({total_samples/sample_rate:.2f}s)")
...
@socketio.on('reset')
def handle_reset():
    """Reset the speech processor context and audio buffer"""
    session_id = request.sid
    try:
        processor = get_speech_processor()
...
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8080))  # Changed default to 8080 to avoid cache
