...
try:
    ...
    try:
        ...
        logger.warning("ffmpeg may not be properly configured")
except ImportError:
    ...
    try:
            ...
            if audio_format == 'pcm':
                ...
                try:
                    ...
                    # Fall through to WebM processing as backup
                    audio_format = 'webm'
            
            # Handle WebM/Opus audio from browser (fallback)
            if audio_format == 'webm' and PYDUB_AVAILABLE:
                # Use pydub to convert WebM to WAV
                temp_webm_path = None
                try:
                    # Try multiple formats and methods
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.webm') as temp_webm:
                        temp_webm.write(audio_bytes)
                        temp_webm_path = temp_webm.name
                    
                    # Try different approaches to load audio
                    audio_segment = None
                    ...
                    if audio_segment is None:
                        ...
                        raise Exception(f"Could not decode audio with any method. "
                                      f"Last error: {error_details[:200]}. "
                                      f"Please ensure ffmpeg supports WebM/Opus: brew install ffmpeg")
                    
                    if audio_segment is None:
                        raise Exception("Failed to load audio segment")
                    ...
                    # Fallback: try to process as raw PCM if possible
                    try:
                        ...
                        if len(audio_bytes) > 0:
                            ...
                            audio_array = audio_array.astype(np.float32) / 32768.0
                            sample_rate = audio_data.get('sample_rate', 16000)
                            logger.info("Using fallback PCM processing")
                        else:
                            raise Exception("Audio data is empty")
                    except Exception as fallback_error:
                        logger.error(f"Fallback processing also failed: {fallback_error}")
                        emit('error', {
                            'message': f'Audio processing failed. Please try again. Error: {str(e)}. '
                                     f'Make sure ffmpeg is properly installed: brew install ffmpeg'
                        })
                        return
                finally:
                    # Clean up temp file
                ...
                # Try raw PCM as last resort
                try:
                    audio_array = np.frombuffer(audio_bytes, dtype=np.int16)
                    audio_array = audio_array.astype(np.float32) / 32768.0
                    sample_rate = audio_data.get('sample_rate', 16000)
                    logger.info("Using raw PCM fallback")
                except Exception as fallback_error:
                    logger.error(f"All audio processing methods failed: {fallback_error}")
                    emit('error', {
                        'message': 'Audio processing failed. Please refresh the page and try again. '
                                 'If the problem persists, check that your microphone is working.'
                    })
                    return
            else:
